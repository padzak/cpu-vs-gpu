...
int main(int argc, char** argv)
{
    ...
    init_rand(tmp); 
    HIP_CHECK(hipMemcpy(d_W1, tmp.data(), tmp.size()*4, hipMemcpyHostToDevice));
    ...
    for (int epoch = 0; epoch < EPOCHS; ++epoch) 
    {
        ...
        while (idx < Nall) 
        {
            ...
            for (int m = 0; m < cur; ++m)
            {
                std::memcpy(&h_X[m * INPUT], dataset[idx + m].pixels.data(), INPUT * sizeof(float));
            }
            ...
            /* -------- forward pass -------- */
            hipLaunchKernelGGL(linear_forward, dim3(cur), dim3(HIDDEN), 0, 0, cur, HIDDEN, INPUT, d_X, d_W1, d_b1, d_H);

            /* ReLU */
            int reluBlocks = (cur * HIDDEN + 255) / 256;
            hipLaunchKernelGGL(relu_kernel, dim3(reluBlocks), dim3(256), 0, 0, d_H, cur * HIDDEN);

            hipLaunchKernelGGL(linear_forward, dim3(cur), dim3(OUTPUT), 0, 0, cur, OUTPUT, HIDDEN, d_H, d_W2, d_b2, d_Y);
            ...
            /* -------- backward W2 -------- */
            hipLaunchKernelGGL(grad_w, dim3(OUTPUT), dim3(HIDDEN), 0, 0, OUTPUT, HIDDEN, d_dW2, d_H, d_dW2, cur);

            /* -------- SGD update W2 only (demo) -------- */
            int saxN = HIDDEN * OUTPUT;
            hipLaunchKernelGGL(saxpy, dim3((saxN + 255)/256), dim3(256), 0, 0, d_W2, d_dW2, -LR / cur, saxN);

            idx += cur;
        }
        std::cout << "Epoch " << (epoch + 1) << "  avg-loss " << epoch_loss / Nall << '\n';
    }
