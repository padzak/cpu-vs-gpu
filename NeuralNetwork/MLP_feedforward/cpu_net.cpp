...
static Mat randn(size_t rows, size_t cols){
    ...
    Mat m(rows, Vec(cols));
    for(auto& r:m) for(float& v:r) v=d(gen);
    return m;
}
...
inline void mmv(const Mat& W, const Vec& x, Vec& y, const Vec& b){
    ...
    for(size_t i=0;i<rows;++i){
        float sum=b[i];
        const float* w=&W[i][0];
        for(size_t j=0;j<cols;++j) sum+=w[j]*x[j];
        y[i]=sum;
    }
}
...
inline void softmax(Vec& v){
    ...
    float sum=0;
    for(float& z:v){ z=std::exp(z-m); sum+=z; }
    for(float& z:v) z/=sum;
}

// main
int main(int argc,char** argv){
    std::string root = argc>1 ? argv[1] : "train";       // dataset folder
    ...
    for(int epoch=0; epoch<EPOCHS; ++epoch){
        ...
        while(idx<N){
            ...
            for(size_t i=idx;i<end;++i){
                ...
                for(int j=0;j<HIDDEN;++j){
                    for(int k=0;k<OUTPUT;++k) gW2[k][j]+=delta2[k]*h_act[j];
                }
                for(int k=0;k<OUTPUT;++k) gb2[k]+=delta2[k];

                // propagate to hidden
                std::fill(grad_h.begin(),grad_h.end(),0);
                for(int j=0;j<HIDDEN;++j){
                    for(int k=0;k<OUTPUT;++k) grad_h[j]+=W2[k][j]*delta2[k];
                ...
                for(int k=0;k<HIDDEN;++k) gb1[k]+=grad_h[k];
            }
            ...
            for(int k=0;k<HIDDEN;++k){
                b1[k]-=scale*gb1[k];
                for(int j=0;j<INPUT;++j) W1[k][j]-=scale*gW1[k][j];
            }
