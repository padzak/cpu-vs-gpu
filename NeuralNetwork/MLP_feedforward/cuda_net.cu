...
int main(int argc,char** argv)
{
    ...
    init_rand(tmp.data(), tmp.size()); 
    CUDA_CHECK(cudaMemcpy(d_W1, tmp.data(), tmp.size() * 4, cudaMemcpyHostToDevice));
    
    tmp.resize(HIDDEN * OUTPUT); init_rand(tmp.data(), tmp.size()); 
    CUDA_CHECK(cudaMemcpy(d_W2, tmp.data(), tmp.size() * 4, cudaMemcpyHostToDevice));
    
    tmp.resize(HIDDEN,0);
    CUDA_CHECK(cudaMemcpy(d_b1, tmp.data(), tmp.size() * 4, cudaMemcpyHostToDevice));
    
    tmp.resize(OUTPUT,0);
    CUDA_CHECK(cudaMemcpy(d_b2, tmp.data(), tmp.size() * 4, cudaMemcpyHostToDevice));
    ...
    for(int epoch = 0; epoch < EPOCHS; ++epoch)
    {
        ...
        while(idx < Nall)
        {
            ...
            for(size_t m = 0; m < cur; ++m)
            {
                ...
                { 
                    row[k]=std::exp(row[k]-maxv); sum+=row[k]; 
                }
                for(int k = 0; k < OUTPUT; ++k) 
                {
                    row[k] /= sum;
